...
export function Footer() {
  ...
  return (
    <footer className="bg-[var(--color-surface)] border-t border-[var(--color-border)] mt-20 sm:mt-28">
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="block text-[var(--color-text-secondary)] hover:text-[var(--color-primary)] transition-colors text-sm"
    >
      {children}
    </Link>
  );
...
function SocialIcon({ label, href, children }: { label: string; href: string; children: React.ReactNode }) {
  return (
    <a
      ...
      className="p-2 rounded-full hover:bg-[var(--color-secondary)] text-[var(--color-text-secondary)] social-hover"
      aria-label={label}
    >
